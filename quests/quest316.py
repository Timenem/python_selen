"""
he businesspeople among you will know that it's often not easy to find an appointment. In this kata we want to find such an appointment automatically. You will be given the calendars of our businessperson and a duration for the meeting. Your task is to find the earliest time, when every businessperson is free for at least that duration.
...
def get_start_time(schedules, duration, day_start='09:00', day_end='19:00'):
    start = time2mins(day_start)
    for s, e in sorted(meeting for calendar in schedules for meeting in calendar):
        if start + duration <= time2mins(s):
            break
        start = max(start, time2mins(e))
    if start + duration <= time2mins(day_end):
        return "%02d:%02d" % divmod(start, 60)  
